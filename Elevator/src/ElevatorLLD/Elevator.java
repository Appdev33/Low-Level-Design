...
public class Elevator {
	...
	/**
	 * UP direction jobs which cannot be processed now so put in pending queue
	 */
	private TreeSet<Request> upPendingJobs = new TreeSet<>();
	/**
	 * DOWN Direction jobs which cannot be processed now so put in pending queue
	 */
	...
	private void addPendingDownJobsToCurrentJobs() {
		if(!downPendingJobs.isEmpty()) {
			currentJobs = downPendingJobs;  //current Jobs point to downPending
	...
	private void addPendingUpJobsToCurrentJobs() {
		if (!upPendingJobs.isEmpty()) {
			...
			currentDirection = Direction.UP;  //current Jobs point to downPending
		} else {
			currentState = State.IDLE;
		}
	}
